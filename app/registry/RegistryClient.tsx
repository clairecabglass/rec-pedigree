  ...
  const filtered = useMemo(() => {
    ...
    const out = horses.filter((h) => {
      ...
      if (breedFilter) {
        ...
        const hb = (h.breed ?? "").toLowerCase();
        // "Warlander X" matches filter "warlander" — strip everything after " x" or " ×"
        const baseBreed = hb.replace(/\s+[x×].*$/, "").trim();
        if (baseBreed !== bf && hb !== bf) return false;
      }
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex flex-col gap-1">
            ...
            <datalist id="breed-list">
              {[...new Set(breeds.map((b) => b.replace(/\s+[xX×].*$/, "").trim()))].sort().map((b) => (
              ...
              <Link key={h.id} href={`/registry/${h.id}`} style={{ textDecoration: "none" }}>
                <div className="hover-card" style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", height: "100%", cursor: "pointer" }}>
                  <div style={{ position: "relative", paddingTop: "78%", background: "linear-gradient(135deg, var(--teal-muted), var(--cream-dark))" }}>
                    {h.photo ? (
                      <Image src={h.photo} alt={h.name} fill quality={90} sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" style={{ objectFit: "cover" }} />
                    ) : (
                    ...
                    {h.ownership && (
                      <span style={{ position: "absolute", top: 8, right: 8, background: (OWNERSHIP_COLORS[h.ownership] ?? DEFAULT_OWNERSHIP_COLOR).bg, color: (OWNERSHIP_COLORS[h.ownership] ?? DEFAULT_OWNERSHIP_COLOR).text, borderRadius: 10, padding: "2px 8px", fontSize: 10, fontWeight: 700, letterSpacing: "0.04em", fontFamily: "var(--font-lato)" }}>
                        {h.ownership}
                      </span>
                    )}
                  </div>
                ...
                <tr key={h.id} style={{ borderBottom: "1px solid var(--border)", background: i % 2 === 0 ? "var(--white)" : "var(--cream)" }}>
                  <td style={{ padding: "6px 10px 6px 14px", width: 44 }}>
                    <div style={{ position: "relative", width: 34, height: 34, borderRadius: 6, overflow: "hidden", background: "var(--teal-muted)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {h.photo ? (
                        <Image src={h.photo} alt="" fill quality={90} sizes="34px" style={{ objectFit: "cover" }} />
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    {h.ownership ? <span style={{ background: (OWNERSHIP_COLORS[h.ownership] ?? DEFAULT_OWNERSHIP_COLOR).bg, color: (OWNERSHIP_COLORS[h.ownership] ?? DEFAULT_OWNERSHIP_COLOR).text, borderRadius: 12, padding: "3px 10px", fontSize: 11, fontWeight: 600 }}>{h.ownership}</span> : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
