...
/* ============================================================================
   Live Show Scoreboard
============================================================================ */

// scoreMode controls the judge panel UI and how scores are stored:
//   "time"         – live timer + penalty accumulator (jumping, xc, barrels)
//   "score-high"   – direct numeric score input, higher wins (dressage %, reining pts)
//   "penalty-only" – penalty accumulator only, no timer, lower wins (eventing total)
type ScoreMode = "time" | "score-high" | "penalty-only";

type Discipline = "jumping" | "xc" | "dressage" | "barrels" | "reining" | "eventing";
...
interface Score {
  ...
  horse: string;
  representing: string;
  // For time/barrels: elapsed ms. For score-high: score × 1000 (so 65.5% → 65500). Unused for penalty-only.
  timeMs: number;
  penalties: number;
  dnf: boolean;
...
interface DisciplineRules {
  ...
  scoreMode: ScoreMode;
  penaltyLabel: string;
  // For score-high: starting value shown in the score input
  defaultScore?: number;
  scoreStep?: number;
  scoreMax?: number;
  scoreUnit?: string; // "%" for dressage, "pts" for reining
  quickActions: { label: string; delta: number; danger?: boolean }[];
  undoLabel?: string;
  undoDelta?: number;
  rankKey: (s: Score) => number[];
  formatPrimary: (s: Score) => string;   // main result column
  formatDetail?: (s: Score) => string;   // detail/deviation column
}
...
const RULES: Record<Discipline, DisciplineRules> = {
  jumping: {
    ...
    undoLabel: "− Undo last 4",
    undoDelta: 4,
    rankKey: (s) => [s.dnf ? 1 : 0, s.penalties, s.timeMs],
    formatPrimary: (s) => s.dnf ? "—" : fmtTime(s.timeMs),
    formatDetail: (s) => s.dnf ? "Eliminated" : s.penalties > 0 ? `${s.penalties} faults` : "Clear",
  },
  xc: {
    label: "Cross Country",
    scoreMode: "time",
    ...
    formatPrimary: (s) => s.dnf ? "—" : fmtTime(s.timeMs),
    formatDetail: (s) => {
      if (s.dnf) return "Eliminated";
      if (s.optimumS == null) return s.penalties > 0 ? `${s.penalties} pen.` : "";
  ...
  dressage: {
    ...
    scoreUnit: "%",
    quickActions: [],
    rankKey: (s) => [s.dnf ? 1 : 0, -s.timeMs], // negate: higher score = lower sort value
    formatPrimary: (s) => s.dnf ? "—" : `${(s.timeMs / 1000).toFixed(1)}%`,
    formatDetail: (s) => {
      if (s.dnf) return "Eliminated";
      const pct = s.timeMs / 1000;
  ...
  barrels: {
    label: "Barrel Racing",
    scoreMode: "time",
    penaltyLabel: "Knockdowns",
    quickActions: [
      { label: "+1 Knocked Barrel (+5s)", delta: 1 },
    ],
    undoLabel: "− Undo knockdown",
    undoDelta: 1,
    // Each knockdown = 5s added to time; off-pattern = DNF
    rankKey: (s) => [s.dnf ? 1 : 0, s.timeMs + s.penalties * 5000],
    formatPrimary: (s) => {
      if (s.dnf) return "—";
      const adj = s.timeMs + s.penalties * 5000;
      return fmtTime(adj);
    },
    formatDetail: (s) => {
      if (s.dnf) return "No Time";
      if (s.penalties === 0) return "Clean run";
      return `${s.penalties} barrel${s.penalties !== 1 ? "s" : ""} (+${s.penalties * 5}s)`;
    },
  },
  reining: {
    label: "Reining",
    ...
    scoreUnit: "pts",
    quickActions: [
      { label: "+1.5 Excellent", delta: 1.5 },
      { label: "+1.0 Very Good", delta: 1 },
      { label: "+0.5 Good", delta: 0.5 },
      { label: "−0.5 Poor", delta: -0.5 },
      { label: "−1.0 Very Poor", delta: -1 },
      { label: "−1.5 Extremely Poor", delta: -1.5 },
    ],
    rankKey: (s) => [s.dnf ? 1 : 0, -s.timeMs],
    formatPrimary: (s) => s.dnf ? "—" : `${(s.timeMs / 1000).toFixed(1)}`,
    formatDetail: (s) => {
      if (s.dnf) return "Eliminated";
      const score = s.timeMs / 1000;
      if (score >= 73) return "Excellent";
  ...
  eventing: {
    label: "Eventing",
    scoreMode: "penalty-only",
    penaltyLabel: "Total Penalties",
    quickActions: [
      { label: "+20 XC Refusal", delta: 20 },
      { label: "+10 XC Time Fault", delta: 10 },
      { label: "+4 SJ Knockdown", delta: 4 },
      { label: "+4 SJ Refusal", delta: 4 },
    ],
    undoLabel: "− Undo last 4",
    undoDelta: 4,
    rankKey: (s) => [s.dnf ? 1 : 0, s.penalties],
    formatPrimary: (s) => s.dnf ? "—" : `${s.penalties} pen.`,
    formatDetail: (s) => {
      if (s.dnf) return "Eliminated";
      if (s.penalties === 0) return "Clean";
      return "";
    },
  },
};
...
export default function ShowScoreboardClient() {
  ...
  const [liveScore, setLiveScore] = useState<number>(RULES.jumping.defaultScore ?? 70); // for score-high mode
  ...
  function resetRun() {
    ...
    setLivePenalties(0);
    setLiveScore(rules.defaultScore ?? 70);
    setLiveDnf(false);
  }
  function addPenalty(n: number) {
    if (rules.scoreMode === "score-high") {
      setLiveScore((s) => Math.min(rules.scoreMax ?? 100, Math.max(0, parseFloat((s + n).toFixed(2)))));
    } else {
  ...
  function submitScore() {
    ...
    pauseTimer();
    const { scoreMode } = rules;
    const finalMs = scoreMode === "score-high"
      ? Math.round(liveScore * 1000)          // store score × 1000 in timeMs
      : scoreMode === "penalty-only"
        ? 0                                    // no time component
        : readElapsed(timer);
    setScores((s) => [
      ...s.filter((sc) => sc.riderId !== activeRider.id),
      {
        ...
        timeMs: finalMs,
        penalties: scoreMode === "score-high" ? 0 : livePenalties,
        dnf: liveDnf,
  ...
  const switchDiscipline = useCallback((d: Discipline) => {
    ...
    setTimer({ running: false, startedAt: null, accumulated: 0 });
    setLivePenalties(0);
    setLiveScore(RULES[d].defaultScore ?? 70);
    setLiveDnf(false);
        ...
        {organizerMode && (
          ...
          >
            ...
            <div className="lg:sticky" style={{ top: 16 }}>
              ...
              {/* ---- Timer (time-based disciplines only) ---- */}
              {rules.scoreMode === "time" && (<>
              ...
              {/* ---- Score input (score-high disciplines: dressage, reining) ---- */}
              {rules.scoreMode === "score-high" && (<>
                <SectionLabel>{rules.penaltyLabel}</SectionLabel>
                <div className="flex items-center gap-2 mb-3">
                  <input
                    type="number"
                    min={0} max={rules.scoreMax ?? 100} step={rules.scoreStep ?? 0.5}
                    value={liveScore}
                    onChange={(e) => setLiveScore(parseFloat(e.target.value) || 0)}
                    disabled={liveDnf || !activeRider}
                    className="w-28 text-2xl font-bold rounded-md border px-2 py-1"
                    style={{ fontFamily: "var(--font-playfair)", borderColor: "var(--border)", color: "var(--teal-dark)", background: liveDnf ? "var(--cream-dark)" : "white" }}
                  />
                  <span style={{ fontFamily: "var(--font-lato)", fontSize: 14, color: "var(--text-muted)" }}>{rules.scoreUnit}</span>
                </div>
              </>)}

              {/* ---- Penalty section (time + penalty-only disciplines) ---- */}
              {rules.scoreMode !== "score-high" && (<>
                <SectionLabel>{rules.penaltyLabel}</SectionLabel>
              ...
              {/* ---- Quick actions (all disciplines that have them) ---- */}
              <div className="flex flex-col gap-2 mb-4">
                ...
                {rules.scoreMode !== "score-high" && (
                  <CtrlBtn onClick={disqualify} disabled={liveDnf || !activeRider} danger>
                    {discipline === "xc" || discipline === "eventing" ? "Disqualify / Fall" : "Disqualify / No Time"}
                  </CtrlBtn>
                )}
                {rules.scoreMode !== "score-high" && rules.undoDelta != null && (
                  <CtrlBtn onClick={() => setLivePenalties((p) => Math.max(0, p - (rules.undoDelta ?? 4)))} disabled={liveDnf || livePenalties === 0}>
                    {rules.undoLabel ?? "− Undo"}
                  </CtrlBtn>
                )}
              </div>
        ...
        <section className="min-w-0">
          ...
          <div className="mt-5 flex items-center justify-between flex-wrap gap-2 mb-2">
            ...
            <button
              ...
              style={{
                background: "var(--sand-bg)", color: "var(--teal-dark)", border: "1px solid var(--sand-border)",
...
function ActiveSpotlight({ rider, timer, running, penalties, liveScore, dnf, rules }: {
  rider: Rider | null; timer: TimerState; running: boolean;
  penalties: number; liveScore: number; dnf: boolean; rules: DisciplineRules;
}) {
  const { scoreMode, scoreUnit, penaltyLabel } = rules;
  return (
    <div style={{
      ...
      <div className="flex items-start sm:items-center justify-between flex-wrap gap-3 mt-1">
        ...
        <div className="text-right shrink-0">
          <div style={{
            ...
            {scoreMode === "score-high"
              ? `${liveScore.toFixed(scoreUnit === "%" ? 1 : 1)}${scoreUnit ?? ""}`
              : scoreMode === "penalty-only"
                ? (dnf ? "DNF" : `${penalties} pen.`)
                : <LiveTimerDisplay timer={timer} />}
          </div>
          {scoreMode === "time" && (
            <div
...
const Leaderboard = forwardRef<HTMLDivElement, LeaderboardProps>(function Leaderboard(
  ...
  return (
    <div ref={ref} style={{
      ...
      <div className="flex items-end justify-between mb-3 pb-2" style={{ borderBottom: "2px solid var(--sand-border)" }}>
        <div>
...
function Row({ place, score, rules, refSetter }: {
  ...
  const primary = rules.formatPrimary(score);
  const dev = rules.formatDetail ? rules.formatDetail(score) : "";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="text-right tabular-nums font-semibold" style={{ color: "var(--teal-dark)" }}>{primary}</div>
        <div className="text-right font-semibold" style={{ color: score.dnf ? "var(--inbreed-text)" : score.penalties > 0 && rules.scoreMode !== "score-high" ? "var(--inbreed-text)" : "var(--text-muted)" }}>
          {score.dnf ? "DNF" : rules.scoreMode === "score-high" ? "—" : score.penalties}
        </div>
        <div className="text-right text-[11px]" style={{ color: "var(--text-muted)" }}>{dev}</div>
      </div>
      ...
      <div className="sm:hidden px-3 py-2.5 flex items-start justify-between gap-2">
        ...
        <div className="text-right shrink-0">
          <div className="tabular-nums text-sm font-semibold" style={{ color: "var(--teal-dark)" }}>{primary}</div>
          <div className="text-xs" style={{ color: "var(--text-muted)" }}>{dev}</div>
        </div>
      </div>
    </div>
  );
