...
export default function BreedingClient({ horses, pregnancies, plans }: { horses: FullHorseData[]; pregnancies: Pregnancy[]; plans: Plan[] }) {
  ...
  const [foalGender, setFoalGender] = useState<string>("Unknown");
  ...
  async function registerPregnancy() {
    ...
    const res = await fetch("/api/pregnancies", {
      ...
      body: JSON.stringify({
        ...
        foalGender: foalGender !== "Unknown" ? foalGender : null,
      }),
    });
    setBusy(false);
  ...
  async function deletePregnancy(id: string) {
    if (!confirm("Remove this pregnancy? The foal placeholder will also be deleted.")) return;
    await fetch(`/api/pregnancies/${id}`, { method: "DELETE" });
    router.refresh();
  }

  async function toggleMarkedForDeletion(id: string, current: boolean) {
    await fetch(`/api/pregnancies/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ markedForDeletion: !current }) });
    router.refresh();
  }
  ...
  const foal = useMemo<HorseNode | null>(() => {
    ...
    return {
      ...
      gender: null,
      coat: null,
      sire: buildPedigreeTree(sire.name, map, 12),
      dam: buildPedigreeTree(dam.name, map, 12),
    };
  }, [dam, sire, map]);
    ...
    const ranked = stallions
      ...
      .map((st) => {
        const f: HorseNode = {
          ...
          sire: buildPedigreeTree(st.name, map, 12),
          dam: buildPedigreeTree(dam.name, map, 12),
        };
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      {/* Suggested Pairings + Calendar links */}
      <div style={{ marginBottom: 28, display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <Link href="/admin/breeding/calendar" style={{ ...secondaryButtonStyle, display: "inline-flex", alignItems: "center", gap: 8 }}>
          <Icon name="tree" size={15} color="var(--teal-dark)" /> Foal Due Calendar
        </Link>
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {pregnancies.map((p) => {
              ...
              const stageColor = stage?.code === "gestation" ? "var(--dam-text)"
                ...
                : "var(--teal-dark)";
              return (
                <div key={p.id} style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-start", gap: 12, border: p.markedForDeletion ? "1px solid #C05050" : "1px solid var(--border)", borderRadius: 8, padding: "12px 16px", background: p.markedForDeletion ? "#FBEEEE" : "var(--cream)", fontFamily: "var(--font-lato)" }}>
                  <div style={{ flex: 1, minWidth: 220 }}>
                    <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                      ...
                      {p.markedForDeletion && (
                        <span style={{ background: "#C05050", color: "white", fontSize: 10.5, letterSpacing: "0.06em", textTransform: "uppercase", fontWeight: 700, borderRadius: 999, padding: "2px 10px" }}>
                          Marked for deletion
                        </span>
                      )}
                    </div>
                  ...
                  <button
                    onClick={() => toggleMarkedForDeletion(p.id, p.markedForDeletion)}
                    title={p.markedForDeletion ? "Unflag — keep this foal" : "Flag this foal to delete once it's born (e.g. unwanted coat/breed)"}
                    style={{
                      background: p.markedForDeletion ? "#C05050" : "none",
                      color: p.markedForDeletion ? "white" : "#C05050",
                      border: "1px solid #C05050", borderRadius: 6, padding: "7px 12px", fontSize: 12, fontWeight: 600,
                      cursor: "pointer", fontFamily: "var(--font-lato)",
                    }}
                  >
                    {p.markedForDeletion ? "Marked for deletion" : "Mark for deletion"}
                  </button>
                  <button onClick={() => deletePregnancy(p.id)} title="Delete now" style={{ background: "none", border: "1px solid var(--border)", color: "#C05050", borderRadius: 6, padding: "7px 12px", fontSize: 12, cursor: "pointer", fontFamily: "var(--font-lato)" }}>✕</button>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 10, padding: 24, marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: wishlistOpen ? 16 : 0, flexWrap: "wrap" }}>
            ...
            <button
              ...
              style={{
                background: "var(--dam-bg)", color: "var(--dam-text)", border: "1.5px solid var(--dam-border)", borderRadius: 8,
        ...
        <>
          ...
          <div style={{ background: "var(--dam-bg)", border: "1px solid var(--dam-border)", borderRadius: 10, padding: "14px 20px", marginBottom: 24, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <label style={{ fontSize: 12, fontFamily: "var(--font-lato)", color: "var(--teal-dark)", fontWeight: 600 }}>Foal gender:</label>
              <select value={foalGender} onChange={(e) => setFoalGender(e.target.value)} style={{ fontSize: 12, fontFamily: "var(--font-lato)", border: "1px solid var(--border)", borderRadius: 6, padding: "5px 8px", background: "var(--white)", color: "var(--teal-dark)" }}>
